...
- Search results use enhanced ranking formula that considers multiple signals:
  ```sql
  ORDER BY (
    bm25(search, 1.0, 5.0, 1.0, 1.0, 1.0, 1.0)  -- Title weight: 5.0
    * boost                                       -- User repo: 2.0, other: 1.0
    * CASE WHEN state = 'open' THEN 1.5 ELSE 1.0 END  -- Open items: 1.5x
    * CASE 
  ...
  ```
- Ranking factors:
  - **Title matches**: 5x BM25 weight (highest priority for text relevance)
  - **User contributions**: 2x boost for items from repositories where user has contributed
  - **Open items**: 1.5x boost (prioritize actionable items)
  - **Recency**: Time decay favors recent items (1.0 for <30d, 0.85 for 30-180d, 0.7 for >180d)
